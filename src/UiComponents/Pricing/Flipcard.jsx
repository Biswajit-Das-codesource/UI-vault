...
const AuroraPricingFlipCards = () => {
  ...
  return (
    <div className="min-h-screen flex flex-wrap justify-center items-center gap-10 bg-black p-6">
      {cardData.map((card, index) => (
        <div key={index} className="relative w-72 h-96 perspective">
          <div
            className={`transition-transform duration-700 ease-in-out w-full h-full relative transform-style-preserve-3d ${flipped[index] ? "rotate-y-180" : ""}`}
          >
            {/* Front Side */}
            <div className="absolute backface-hidden w-full h-full bg-[#111] rounded-2xl border border-white p-6 flex flex-col items-center justify-between shadow-[0_0_30px_#ffffff15]">
              <img src={card.img} alt="Profile" className="w-20 h-20 rounded-full border-2 border-white" />
            ...
            {/* Back Side */}
            <div className="absolute rotate-y-180 backface-hidden w-full h-full bg-[#111] rounded-2xl border border-white p-6 flex flex-col items-center justify-between shadow-[0_0_30px_#ffffff15]">
              <motion.img
                src={card.qr}
                alt="QR Code"
                className="w-40 h-40"
