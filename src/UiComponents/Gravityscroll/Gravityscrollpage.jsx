...
function GravityScrollPage() {
  ...
  const gravityCode = `import './App.css'
import GravityScroll from './UIvault/gravity/GravityScroll';
  ...
  return (
    <div>
      ...
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-bold">GravityScroll (Scroll Animation)</h1>
        <p className="text-gray-400 mt-1">
          Immersive scroll-based split animation with pinned sections using GSAP.
        </p>
        ...
        <div className="flex items-center justify-between bg-[#1a1a1a] border border-gray-700 text-green-400 px-5 py-4 text-sm rounded-2xl mt-4">
          <code className="geist-mono overflow-x-auto text-sm" style={{ letterSpacing: "1px" }}>
            <span className="text-cyan-400">npx</span> ui-vault add gravityscroll
          </code>
          <button
            onClick={() => handleCopy("npx ui-vault add gravityscroll", "gravity-cli")}
            className="ml-4 text-white transition"
            title="Copy to clipboard"
          >
            {copied === "gravity-cli" ? <ClipboardCheck size={18} /> : <Clipboard size={18} />}
          </button>
        </div>
        ...
        <div className="flex border-b border-white/10 mt-5">
          <button
            ...
            className={`px-4 py-2 font-medium ${
              tab === "preview" ? "text-white border-b-2 border-white" : "text-white/50"
            }`}
          >
            Preview
          </button>
          <button
            onClick={() => setTab("code")}
            className={`px-4 py-2 font-medium ${
              tab === "code" ? "text-white border-b-2 border-white" : "text-white/50"
            }`}
          >
            Code
          </button>
        ...
        {tab === "preview" && (
          <div className="mt-5 bg-black h-screen w-full">
            <GravityScroll />
          </div>
        )}
...
export default GravityScrollPage;
